...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.csharp.data.mysql.load">
        ...
        <xsl:when test="@use = 'optional'">
          ...
          <xsl:choose>
            ...
            <xsl:when test="@type = 'xs:boolean'">
              <xsl:value-of select="$functionParam" />.<xsl:value-of select="$property"/> = ((sbyte)dataReader.GetValue(<xsl:value-of select="position()-1" />)) == 1;
            </xsl:when>
            <xsl:otherwise>
    ...
    protected override List&lt;<xsl:value-of select="$class-name" />&gt; fill(MySqlCommand sqlCommand, int offset, int limit)
    {
            ...
            foreach(DataRow dataRow in dataTable.Rows)
            {
              ...
              <xsl:for-each select="xs:complexType/xs:attribute">
                ...
                <xsl:choose>
                  ...
                  <xsl:when test="@type = 'xs:boolean'">
                    item.<xsl:value-of select="$property"/> = ((sbyte)dataRow["<xsl:value-of select="@name"/>"]) == 1;
                  </xsl:when>
                  <xsl:otherwise>
    ...
    <xsl:for-each select="xs:keyref">
      ...
      private const String <xsl:value-of select="$sql-name"/> = @"select * from `<xsl:value-of select="$table" />`
      where
      ...
      using(MySqlCommand sqlCommand = Database.CreateCommand(<xsl:value-of select="$sql-name"/>))
      {
      <xsl:for-each select="xs:field">
