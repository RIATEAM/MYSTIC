...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.csharp.data.mysql.update">
    ...
    <xsl:if test="$editable">
      const String SqlUpdate = @"Update `<xsl:value-of select="@name" />` Set
    ...
    <xsl:if test="$editable">
      ...
      {
        using(MySqlCommand sqlCommand = Database.CreateCommand(SqlUpdate))
        {
